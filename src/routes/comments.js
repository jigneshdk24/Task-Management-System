...
const { createComment, updateComment, deleteComment, getCommentById, listTaskComments, listAllComments } = require("../controllers/commentsController");
...
router.get("/comments", authMiddleware, (req, res, next) => {
	return req.user.is_admin === 1 ? next() : res.status(403).json({ message: "Forbidden" });
}, listAllComments);
...
router.post("/tasks/:taskId/comments", authMiddleware, prepareTaskComment, commentRbac, createComment);
